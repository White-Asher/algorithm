	...
	public static void simultate() {
		...
		for (int time = 0; time < MAP_SIZE + 5; time++) {
			...
			for (int index = 0; index < atomSize; index++) {
				...
				// 먼저 원자 리스트에 이동한 좌표 넣기 
				atomList[index].y = ny;
				atomList[index].x = nx;
				
				// 맵 밖을 안 나간 원자들 이동한 곳 맵에 표시
				if(!isNotArea(ny, nx)) {
					map[ny][nx] += 1;
			...
			for (int index = 0; index < atomSize; index++) {
				...
				if(collision[curY][curX] == true) {
					if(map[curY][curX] == 1) collision[curY][curX] = false;
